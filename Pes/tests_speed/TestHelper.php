...
//    }
    ...
    public function vypisModel(Framework_Model_Persistable_ItemInterface $item) {
        return $this->vypisModelsArray(array($item));
    }
    
    public function vypisModelsArray(array $models) {

        if ($models) {
            $html[] = "<p>pole modelů ".get_class(end($models)).":</p>";
            $html[] ="<table border=1>";
            $prvniItem = TRUE;
            foreach ($models as $item) {
                if ($prvniItem) {
                    $prvniItem = FALSE;
                    $html[] = "<tr>";
                    $html[] = $this->vypisHlavickuProItem($item);
                    $html[] ="</tr>";
                }
                $html[] ="<tr>";
                foreach ($item as $polozka)
                    if (is_object($polozka))
                        $html[] ="<td>".get_class($polozka)."</td>";
                    else
                        $html[] ="<td>{$polozka}</td>";
                $html[] ="</tr>";
            }
    ...
    /**
     * @param Framework_Model_Persistable_ItemInterface $item
     * @return type
     */
    public function vypisHlavickuProItem(Framework_Model_Persistable_ItemInterface $item) {
        $html = array();
    ...
    /**
     * Opsáno z Framework_View_Abstract->__toString()
     * @param array $htmlArray
     * @return string
     */
    public function convertToString(array $htmlArray) {
        if ($htmlArray) {
