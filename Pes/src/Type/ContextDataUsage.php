...
class ContextDataUsage {
    ...
    public function logStatus($logFileName, ContextDataInterface $contextData) {
        ...
        $statusLogger->debug('Historie volání proměnných kontextu '.$statusLogger->getLogFilePath().':');
        if ($status) {
        ...
        if ($warning) {
            $statusLogger->warning(print_r($warning, TRUE));      
        }
        if ($notice) {
            $statusLogger->notice(print_r($notice, TRUE));      
        }            
        if ($debug) {
            $statusLogger->debug(print_r($debug, TRUE));   
        }
        if ($info) {
            $statusLogger->info(print_r($info, TRUE));      
        }
        if ($notice) {
