<?php
class Framework_Storage_CryptCookie extends Framework_Storage_StorageAbstract implements Framework_Storage_StorageInterface {
    ...
    public function get($key) {
        ...
        if (isset(self::$cookies[$index])) {
            $this->_unpack(self::$cookies[$index]);
            return $this->content;
        } elseif (isset($_COOKIE[$index])) {
            $this->_unpack($_COOKIE[$index]);
            return $this->content;
        }
        return FALSE;
    }
    ...
    private function _pack() {
        $parts = array($this->version, $this->created, $this->valueToStore($this->content));
    ...
    private function _unpack($cookie) {
        ...
        $this->content = $this->valueRestored($content);
