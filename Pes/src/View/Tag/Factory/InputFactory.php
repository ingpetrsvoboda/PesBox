...
namespace Pes\View\Tag\Factory;

use Pes\View\Tag;
use Pes\View\Tag\Attributes;
...
class InputFactory {
    public static function text($context, $name, $readonly=FALSE, $maxlength=NULL, $size=ŃULL) {
        ...
        $inputAttributes = (new Attributes\InputAttributes())->setAttributesArray(array('id'=>$name, 'type'=>'text', 'name'=>$name, 'size'=>$size, 'maxlength'=>$maxlength, 'value'=>$context[$name]));
        if ($readonly) {
